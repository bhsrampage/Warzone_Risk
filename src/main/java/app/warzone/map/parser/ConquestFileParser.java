...
import static app.warzone.map.parser.MapFileParser.getStrings;

/**
 * The ConquestFileParser class parses a Conquest map file and constructs a map based
 * on its contents.
 *
 * @author Burhanuddin
 */
public class ConquestFileParser {
    /**
    ...
    /**
     * Constructs a ConquestFileParser for parsing a conquest map file with the given name.
     *
     * @param name The name of the map.
     */
    public ConquestFileParser(String name) {
        d_parsedMap = new Map(name,"Conquest");
    }
    ...
    void parseContinents(StringBuilder p_fileText) {
        String[] l_continentLines = lineExtractor(p_fileText, "[Continents]", "[Territories]");
        int i = 1;
    ...
    void parseCountries(StringBuilder p_fileText) {
        String[] l_countryLines = lineExtractor(p_fileText, "[Territories]", null);
        int i = 1;
        for (String l_countryInfo : l_countryLines) {
            String[] l_infoTokens = l_countryInfo.split(",");
            if (l_infoTokens.length < 5) continue;
            d_parsedMap.addCountry(new Country(
                    i,
                    l_infoTokens[0],
                    d_parsedMap.getContinentByName(l_infoTokens[3])
                    )
            );
            i++;
        }
    ...
    void parseConnections(StringBuilder p_fileText) {
        String[] l_connectionLines = lineExtractor(p_fileText, "[Territories]", null);
        for (String l_connectionInfo : l_connectionLines) {
            String[] l_infoTokens = l_connectionInfo.split(",");
            if (l_infoTokens.length < 5)
                continue;
            Country l_targetCountry = d_parsedMap.getCountryByName(l_infoTokens[0]);
            for (int l_i = 4; l_i < l_infoTokens.length; l_i++) {
                l_targetCountry.addRemoveNeighbour(d_parsedMap.getCountryByName(l_infoTokens[l_i]),
                        true);
            }
        }
    }
