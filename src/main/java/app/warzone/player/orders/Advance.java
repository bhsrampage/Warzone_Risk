...
public class Advance extends Order {
	...
	public boolean isValid() {
		...
		if (d_armyCount > d_sourceCountry.getCurrentArmyCount()) {
			System.out.println(
					d_attackingPlayer.d_playerName + " is trying to advance more armies than the current army count of "
							+ d_sourceCountry.getD_countryName());
			return false;
		}
		if (!d_sourceCountry.getNeighbouringCountries().contains(d_targetCountry)) {
			System.out.println(
					d_targetCountry.getD_countryName() + " is not adjacent to " + d_sourceCountry.getD_countryName());
			return false;
		}
		if (d_targetCountry == d_sourceCountry) {
			System.out.println(d_targetCountry.getD_countryName() + " cannot be the same as "
					+ d_sourceCountry.getD_countryName());
			return false;
		}
		return true;
	...
	public void execute() {
		...
		if (isValid()) {
			if (d_targetCountry.getCountryHolder() == d_sourceCountry.getCountryHolder()) {
				...
				System.out.printf("\nMoved %d armies to %s from %s", d_armyCount, d_targetCountry.getD_countryName(),
						d_sourceCountry.getD_countryName());
			} else {
				...
				if (l_defendingNum == 0) {
					...
					System.out.println(d_attackingPlayer.d_playerName + " has lost the battle for "
							+ d_targetCountry.getD_countryName());
				}
			}
	...
	public static boolean generateBooleanWithProbability(double p_probability) {
		...
		double l_randomValue = l_random.nextDouble(); // Generates a random number between 0.0 (inclusive) and 1.0
														// (exclusive).
